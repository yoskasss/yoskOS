...
void get_cpu_model(char* model) {
    ...
    // Simple string building without sprintf
...
// Get memory info (simplified, assume fixed for now)
uint32_t get_memory_size() {
    // In a real OS, this would read from CMOS or use multiboot info
    // For simplicity, return a fixed value or try to detect
